...
print(f'V_POI = {value/1000:0.3f} (modbus value = {value})')
reg_number = 370
value = mb.read(reg_number, 'int32',format = 'CDAB')
print(f'P_POI = {value*100e6*1e-6/1e6:0.3f} MW  (modbus value = {value})')
mb.close()
reg_number = 374
value = mb.read(reg_number, 'int32',format = 'CDAB')
print(f'Q_POI = {value*100e6*1e-6/1e6:0.3f} Mvar  (modbus value = {value})')
mb.close()
...
for m in range(M):
    for n in range(N):
        ...
        value = p_ppc
        address = f'{reg_number}@{ip}:{port}'
        print(f'{name}: p_ppc = {p_ppc/1000:5.2f} kW  (modbus value = {value}), address = {address}')
        mb.close()
...
for m in range(M):
    for n in range(N):
        ...
        mb.close()
        address = f'{reg_number}@{ip}:{port}'
        print(f'{name}: P = {p/1000:5.2f} kW  (modbus value = {value}), address = {address}')
...
for m in range(M):
    for n in range(N):
        ...
        value = q_ppc
        address = f'{reg_number}@{ip}:{port}'
        print(f'{name}: q_ppc = {q_ppc/1000:5.2f} kW  (modbus value = {value}), address = {address}')
        mb.close()
...
for m in range(M):
    for n in range(N):
        ...
        mb.close()
        address = f'{reg_number}@{ip}:{port}'
        print(f'{name}: Q = {value/1000:5.2f} kvar  (modbus value = {value}), address = {address} ')
