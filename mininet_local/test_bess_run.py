...
This file have to be run within inside mininet: 
    ...
    mininet> PPC python3 ./mininet_local/test_bess.py
...
for it in range(1000):
    ...
    # active power reference
        p_ref = 1.0e6*np.sin(2*np.pi*2.0*t)
    if it > 500:
    # active power reference
        p_ref = 0.0e6

    reg_number = 40424
    mb.write(int(p_ref), reg_number, 'int32',format = 'CDAB')
    ...
    t = time.time()-t_0
    #print(f'{name}: t = {t:6.4f} s, p_ref  = {p_ref/1000:5.2f} kW,  P = {p/1000:5.2f} kW')
    record += [{'t':t, 'p_ref':p_ref,  'p_s':p}]

    time.sleep(0.01)

with open('record_bess.json', 'w') as fobj:
    json.dump(record, fobj, indent=4)  # 'indent' makes the file readable
    

# soc  measurement
reg_number = 40548
p = mb.read(reg_number, 'int32',format = 'CDAB')
print(f'{name}: soc = {p/10:5.2f} %')
mb.close()
